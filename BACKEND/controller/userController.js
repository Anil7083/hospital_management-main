...
export const patientRegister = catchAsyncErrors(async (req, res, next) => {
    ...
    if (isRegistered) {
        return next(new ErrorHandler("User Already Registared", 400));
    }
...
export const adddNewDoctor = catchAsyncErrors(async (req, res, next) => {
    if (!req.files || Object.keys(req.files).length === 0) {
        return next(new ErrorHandler("Doctor Avatar Required", 400));
    }
    const { docAvatar } = req.files;
    const allowedFormats = ["image/png", "image/jpeg", "image/webg"];
    if (!allowedFormats.includes(docAvatar.mimetype)) {
        return next(new ErrorHandler("File Format Not Supported", 400));
    }
    const { firstName, lastName, email, phone, password, gender, dob, nic, doctorsDepartment } = req.body;
    if (!firstName || !lastName || !email || !phone || !password || !gender || !dob || !nic || !doctorsDepartment || !docAvatar) {
    ...
    const doctor = await User.create({
        firstName, lastName, email, phone, password, gender, dob, nic, doctorsDepartment,
